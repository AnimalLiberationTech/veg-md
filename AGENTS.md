...
- Appwrite analytics: telemetry writes to the `analytics` database / `user-actions` table and expects fields like `event_name`, `path`, `referrer`, `device_type`, `country`, `metadata`, `user_agent`, `site`, and `env`.
